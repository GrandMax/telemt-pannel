...
### 5.2.1 Unique IPs (текущие подключения)

- В telemt в памяти хранится множество активных IP по каждому пользователю: при подключении клиента вызывается `check_and_add(username, ip)`, при отключении — `remove_ip(username, ip)`.
- Метрика `telemt_user_unique_ips_active{user="..."}` — это **текущее** число уникальных IP, с которых есть активные соединения (gauge), а не накопительная статистика.
- Панель раз в **30 секунд** скрейпит метрики и записывает значение в `user.active_unique_ips`; в UI отображается последнее сохранённое значение.
- Значение обнуляется: (1) когда все клиенты пользователя отключились; (2) после рестарта telemt (счётчики только in-memory). Лимит одновременных уникальных IP задаётся в настройках пользователя (`max_unique_ips`).
