  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--min-success":
        ...
        break;
      case "--help":
        console.log("Usage: node direct_load.js [--host 127.0.0.1] [--port 443] [--clients 10] [--duration 15] [--connect-timeout-ms 5000] [--stall-ms 5000] [--max-failures 0] [--max-timeouts 0] [--min-success 1]");
        process.exit(0);
      default:
...
async function oneAttempt(options, stats) {
  ...
  return new Promise((resolve) => {
    ...
    socket.on("connect", () => {
      setTimeout(() => finish("success", "idle-stall-complete"), options.stallMs);
    });

    socket.on("error", (error) => {
      finish("error", `connect:${error.message}`);
    });
  });
}
...
async function main() {
  ...
  console.log(`step: direct relay load start host=${options.host} port=${options.port} clients=${options.clients} duration=${options.duration}s stall=${options.stallMs}ms`);
